...
Say hello to my homelab!
The repository holds the complete configuration for all my homelab devices and services. It is meant as a single point of configuration for everything that has to do with my homelab. No manual configuration should be necessary.
It has the following important subdirectories:

- infrastructure - infrastructure deployment files and folders
- gitops - ArgoCD app location
- docs -- contains all relevant documentation


## 🤯 Features

- [ ] Kubernetes cluster deployment using TalosOS
- [ ] Infrastructure Automation with Ansible to provision hosts, clusters, devices, etc.
- [ ] Offline Root CA / Scripted PKI management using `smallstep`
...
You need to have the following apps installed to use and development this homelabd

- [ ] PDM - Manual install necessary - ```https://pdm-project.org/en/latest/#__tabbed_5_2```
- [ ] Ansible - automatically installed through PDM - ```https://docs.ansible.com/ansible/latest/installation_guide/intro_installation.html```
- [ ] Terraform - manual install necessary - ```https://developer.hashicorp.com/terraform/install?product_intent=terraform```

The repository for prepared for python developing, by executing the following commands:
```console
pdm init # choose current python version and say yes to creating a venv
```
